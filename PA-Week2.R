...
setwd("E:/rechal/Coursera/DataScienceSpecialization/5-Reproducible Research")
...
# Some days have no data, so it is not feasible to replace missing values with the day's mean. Instead, we can replace missing values with the mean number of steps for each interval across all of the days. The act.int data frame contains these means. Start by merging the act.int data with the raw data:
...
# Convert the date variable to the date class, then use the weekdays() function to generate the day of the week of each date. Create a binary factor to indicate the two weekend days:
...
# First create separate data frames for weekends and weekdays:
